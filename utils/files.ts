 ...
 * @param {string} dest - Where to move the file.
 * @returns {string}
 */
export function getNotExistingFilename(currentFilename: string, dest: string) {
  ...
  const checkNumber = async (current: number): Promise<string> => {
    ...
    const doesExist = await exists(resolve(dest, filename));
